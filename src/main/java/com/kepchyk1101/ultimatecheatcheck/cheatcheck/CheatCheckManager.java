...
import com.kepchyk1101.ultimatecheatcheck.events.*;
...
import com.kepchyk1101.ultimatecheatcheck.utils.EventUtils;
...
public class CheatCheckManager {
        ...
        if (!activeChecks.containsKey(suspect)) {
            ...
            if (!suspect.hasPermission("ucc.immunity")) {
                ...
                if (activeChecks.size() == 1)
                    Bukkit.getPluginManager().registerEvents(new AllListeners(), UltimateCheatCheck.getInstance());
    ...
    public static void completionAllChecks() {
        ...
        if (activeChecks.size() >= 1) {
            ...
            UltimateCheatCheck.getLog().info(ConfigUtils.getMessage("system.completionAllChecks"));
